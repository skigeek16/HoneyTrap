...
class ScamDetectionEngine:
    ...
    def _calculate_ensemble_score(self, rule: Dict, ml: Dict) -> float:
        ...
        score = (
            (ml['phishing_prob'] * settings.WEIGHT_PHISHING) +
            (rule['rule_score'] * settings.WEIGHT_RULE_BASED) +
            (ml['multilingual_scam_prob'] * settings.WEIGHT_MULTILINGUAL)
        )
        if ml['emotions'].get('fear', 0) > 0.3 or ml['emotions'].get('joy', 0) > 0.7: score += 15.0
        if ml['intent'] not in ["innocent_conversation"]: score += (ml['intent_conf'] * 100 * settings.WEIGHT_INTENT)
